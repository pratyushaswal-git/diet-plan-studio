...
- **First batch is repo setup.** Initialize the repo and set the git identity (repo-local, not global):
  ...
  ```
  **Updated (Batch 10.1):** commits in this repo are now authored as
  `Nuvira Fertility <nuvirafertility@gmail.com>` (repo-local config). Vercel's
  **Hobby** plan only deploys a private repo when the head commit's author is the
  Vercel account owner — so the author must be the Nuvira identity, not Pratyush.
  Keep the `Co-Authored-By: Claude` trailer; only the author changed.
...
| **PDF generation** | **headless Chrome** (`puppeteer-core` + `@sparticuz/chromium`; `puppeteer` in dev) | One React `<PlanView>` HTML/CSS template → live scaled preview **and**, via `renderToStaticMarkup` → headless Chrome → a content-sized PDF, the download. Pixel-faithful to the locked design (gradients, radial wash). Rendered PDFs are **cached** in `plan-pdfs` (keyed by plan `updated_at`); recipe cards are **text/offline** (Batch 11, no thumbnail fetches). Replaced `@react-pdf/renderer` in Batch 9 (its engine couldn't paint the design). |
...
- **Batch 10.1 — Vercel deploy unblocked (commit author).** First deploy was blocked: Vercel **Hobby** won't deploy a **private** repo unless the head commit's author is the Vercel account owner, and commits were authored as Pratyush. Repo must stay private (client PDFs in `plans/`), so going public was out; coach chose the free fix over Pro. Set repo-local git author to `Nuvira Fertility <nuvirafertility@gmail.com>` (the Nuvira Vercel/GitHub owner identity) so the deployed head commit passes the author check. All future commits in this repo use that author; `Co-Authored-By: Claude` trailer kept. Hobby limits still apply (~1 GB / 60 s, non-commercial) — if the headless-Chrome PDF Download OOMs/times out, upgrade to Pro.
...
- **Fix — PDF Download returned 500 on Vercel.** Root cause (surfaced by wrapping the route in try/catch → `console.error` + returning the real error text): `@sparticuz/chromium` ships Chromium in `node_modules/@sparticuz/chromium/bin/*.br` and extracts it **by path at runtime**, so Next's file tracer never included it → `executablePath()` threw "input directory …/@sparticuz/chromium/bin does not exist". Fix: force-include it in the route's trace — `experimental.outputFileTracingIncludes["/api/plans/[id]/pdf"]` now lists `./node_modules/@sparticuz/chromium/bin/**` (alongside `./public/fonts/**`). **Confirmed: Download produces the PDF on production (Hobby).** The verbose error-surfacing in `route.tsx` is kept (admin-only route) for future host-render debugging. (If the render ever OOMs/times out under heavier plans, Hobby's ~1 GB/60 s is the ceiling → Pro.)

### Batch 11 — Cheaper/lighter PDF render (cache + text recipe cards + font trim + browser reuse) (2026-06-15)
- **Why:** the Download launches headless Chromium per request — costly on Hobby (~1 GB/60 s). Reduced cost/memory with no design-quality loss (coach accepted text recipe cards).
- **Text-only recipe cards:** `PlanView.tsx` recipe map drops the `<img>` (no per-recipe YouTube thumbnail fetch/decode/wait); card keeps the tinted band + play badge + Short/Video tag + title + Watch. `plan-view-css.ts` drops `.rcard__thumb::after`. **Reverses Batch 9's real-thumbnails decision** (coach's call). The brand logo is still an `<img>`, so a (lighter) image-wait remains.
- **PDF caching:** `app/api/plans/[id]/pdf/route.tsx` now checks `plan-pdfs` for `${id}/${updated_at-ms}.pdf` via the service client → serves the stored file (no Chromium) on hit; on miss renders, uploads, and best-effort prunes older versions for that plan. `updated_at` in the path auto-invalidates on edit. No schema change (`plans.pdf_url` still unused). Bucket stays private; served only through this auth-gated route.
- **Font trim 13→7:** only the used weights remain — Cormorant 500/600 + italic-500, Mulish 400/600/700/800. Trimmed `render-pdf.ts` `FACES`, `plan-view-css.ts` `@font-face`, and deleted 6 TTFs. Smaller inlined HTML, less parse/memory; zero visual change.
...
- **Custom domain (LIVE):** the app is live at **`https://dietstudio.nuvirahealth.in`** (CNAME `dietstudio` → `cname.vercel-dns.com` at the `nuvirahealth.in` DNS host; Vercel auto-TLS). Set as the **Primary** domain in Vercel; the old `*.vercel.app` URL is configured to **308-redirect** to it (Settings → Domains → the vercel.app domain → Redirect to → 308). No code/deploy for the domain.
- **No Supabase URL config needed:** auth is **`signInWithPassword` only** (cookie sessions; no OAuth / magic-link / password-reset / email-confirmation), so Supabase's Site URL & Redirect URLs settings don't apply and login works on any host. (Only set them if an email/OAuth flow is added later.)
