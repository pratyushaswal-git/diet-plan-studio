...
- **Subdomain (coach action, no code):** to serve the app at **`dietstudio.nuvirahealth.in`** — Vercel → project → Settings → Domains → add the subdomain → create the shown CNAME (`dietstudio` → `cname.vercel-dns.com`) at the `nuvirahealth.in` DNS host; Vercel auto-issues TLS. No app/env/Supabase change needed — relative URLs + cookie auth work on any host.
