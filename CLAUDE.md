...
- **Custom domain (LIVE):** the app is live at **`https://dietstudio.nuvirahealth.in`** (CNAME `dietstudio` → `cname.vercel-dns.com` at the `nuvirahealth.in` DNS host; Vercel auto-TLS). Set as the **Primary** domain in Vercel; the old `*.vercel.app` URL **308-redirects** to it (**done**). No code/deploy for the domain.
- **Supabase URL config:** not required for auth (it's **`signInWithPassword` only** — cookie sessions, no OAuth/magic-link/reset redirects). Site URL set to the custom domain anyway as future-proofing (**done**).
